...
class Env:
    ...
    def _is_episode_completed(self, episode_ndx):
        """检查某个 episode 是否已经成功完成（用于断点续跑）"""
        episode = self.all_episodes[episode_ndx]
        if 'hm3d' in self.cfg.get('dataset', ''):
            f = episode['scene_id'].split('/')[1:]
            scene_id = f[1][2:5]
        elif 'mp3d' in self.cfg.get('dataset', ''):
            scene_id = episode['scene_id'].split('/')[1]
        else:
            return False

        curr_run_name = f"{episode_ndx}_{scene_id}"
        pkl_path = os.path.join(os.environ.get("LOG_DIR"),f'{self.outer_run_name}/{self.inner_run_name}/{curr_run_name}/df_results.pkl')
        if not os.path.exists(pkl_path):
            return False
        try:
            df = pd.read_pickle(pkl_path)
            if len(df) > 0 and 'done' in df.columns:
                return bool(df.iloc[-1].get('done', False))
        except Exception:
            return False
        return False
    def run_experiment(self):
        ...
        skip_to = self.cfg.get('start_episode', None)
        target_scene = self.cfg.get('target_scene_id', None)
        ...
        for episode_ndx in range(start_ndx, min(start_ndx + self.cfg['num_episodes'], end_ndx)):
            ...
            if self.cfg.get('resume', False) and self._is_episode_completed(episode_ndx):
                logging.info(f'Episode {episode_ndx} already completed, skipping.')
                continue
...
class WMNavEnv(Env):
    ...
    def _initialize_episode(self, episode_ndx: int):
        ...
        if os.path.exists(episode_log_dir):
            ...
            logging.info(f'已清除当前episode旧日志: {episode_log_dir}')
    # ===== 新增结束 =====
    ...
    def _step_env(self, obs: dict):
        ...
        # ── 模块二: 更新工作记忆计数器 ──
        if hasattr(self.agent, 'steps_since_door_entry'):
            self.agent.steps_since_door_entry += 1

        # ── 模块三: 记录位置 & 困死检测 ──
        if hasattr(self.agent, 'update_position_history'):
            self.agent.update_position_history(obs['agent_state'])
        if hasattr(self.agent, 'detect_stuck'):
            _stuck = self.agent.detect_stuck()
            if _stuck == 2:
                self.agent.init_global_recovery()
                logging.info("[Env] Global recovery triggered")

        # ── 模块二: 区域穷尽检测 ──
        if hasattr(self.agent, 'check_room_exhaustion'):
            self.agent.check_room_exhaustion(obs['agent_state'])

        # ── 模块三: 目标锚定偏置 ──
        if hasattr(self.agent, 'apply_target_anchor_bias'):
            self.agent.apply_target_anchor_bias(obs['agent_state'])
        ...
        # ── 模块二: 门记忆 & 场景检测 ──
        if hasattr(self.agent, 'record_door_position'):
            _subtask_lower = str(subtask).lower()
            if any(kw in _subtask_lower for kw in ['door', 'hallway', 'corridor', 'entrance', 'opening']):
                self.agent.record_door_position(obs['agent_state'])

        if (hasattr(self.agent, 'steps_since_door_entry') and
            self.agent.steps_since_door_entry == 3 and
            hasattr(self.agent, 'detect_scene_type_from_vlm')):
            self.agent.detect_scene_type_from_vlm(direction_image, self.current_episode['object'])
            if hasattr(self.agent, 'check_scene_mismatch_and_block'):
                self.agent.check_scene_mismatch_and_block(obs['agent_state'])
